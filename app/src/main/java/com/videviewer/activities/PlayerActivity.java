...
public class PlayerActivity extends AppCompatActivity {
    ...
    private void loadVideo() {
        try {
            ...
            binding.tvVideoTitle.setText(videoTitle != null ? videoTitle : uri.getLastPathSegment());
            player.setMediaItem(MediaItem.fromUri(uri));
            player.prepare();
            player.play();

            // Show resume snackbar if there's a saved position > 5 seconds
            if (savedResumePosition > 5000) {
                final long resumePos = savedResumePosition;
                handler.postDelayed(() -> {
                    if (player != null && binding != null) {
                        Snackbar.make(binding.getRoot(),
                            "Resume from " + formatTime(resumePos) + "?",
                            Snackbar.LENGTH_LONG)
                            .setAction("Resume", v -> player.seekTo(resumePos))
                            .show();
                    }
                }, 1000);
            }
    ...
    private void showControls() {
        ...
        binding.topBar.animate().alpha(1f).setDuration(250)
            .withStartAction(() -> binding.topBar.setVisibility(View.VISIBLE)).start();
        binding.bottomBar.animate().alpha(1f).setDuration(250)
            .withStartAction(() -> binding.bottomBar.setVisibility(View.VISIBLE)).start();
        scheduleHideControls();
    }
    ...
    private void hideControls() {
        ...
        binding.bottomBar.animate().alpha(0f).setDuration(250)
            .withEndAction(() -> binding.bottomBar.setVisibility(View.GONE)).start();
    }
